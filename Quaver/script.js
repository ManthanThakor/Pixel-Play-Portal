...
function startVisualizer() {
    ...
    function draw() {
        ...
        for (let i = 0; i < bufferLength; i++) {
            const percent = dataArray[i] / 255; // Normalize the value to a percentage
            const radius = percent * (canvas.width / 4); // Scale the radius based on the canvas size

            canvasCtx.beginPath();
            canvasCtx.arc(centerX, centerY, radius, 0, 2 * Math.PI);
            canvasCtx.strokeStyle = `rgba(255, 255, 255, ${percent})`; // Set color and transparency based on intensity
            canvasCtx.stroke();
        }
    }
